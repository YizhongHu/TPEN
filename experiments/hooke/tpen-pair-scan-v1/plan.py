...
The planner also owns config *composition*. The scan's ``configs/train.yaml``
and ``configs/eval.yaml`` are deliberately not self-contained: ``choices.basis``
lives in one shared table declared by the grid's ``choice_libraries``. This
script merges those fragments in, asserts the required choice paths are
populated, writes the composed result as the grid attempt's config snapshot, and
compiles every command against that snapshot -- never against the un-merged
source file. See ``utils/config.py`` for the merge contract.
...
choice_library_provides = _tpen_utils_config.choice_library_provides
choice_library_specs = _tpen_utils_config.choice_library_specs
...
require_choice_paths = _tpen_utils_config.require_choice_paths
...
def required_choice_paths(grid_data: dict[str, Any]) -> list[str]:
    """Return every config path that must be populated before commands compile.

    The union of two independent declarations, so neither one alone can let an
    un-merged config through:

    * ``choice_libraries[].provides`` -- what each shared fragment is expected
      to define, i.e. the merge the launcher owes the run config.
    * ``choice_validation[].choices_path`` -- what the planner validates grid
      values against.

    A grid that declares ``choices.basis`` in either place therefore cannot
    plan a lineage whose ``choices.basis`` is missing.
    """

    paths = choice_library_provides(choice_library_specs(grid_data.get("choice_libraries")))
    for spec in choice_validation_specs(grid_data).values():
        candidate = str(spec.get("choices_path", "")).strip()
...
def build_manifest(
    *,
    ...
    # Record which shared fragments were composed into the snapshots this
    # attempt's commands run, so a reader can check the merge happened without
    # re-deriving it from the study's source configs.
    manifest["choice_libraries"] = choice_library_specs(grid_data.get("choice_libraries"))
    manifest["required_choice_paths"] = [
        str(path) for path in (grid_data.get("required_choice_paths") or ())
    ]
    return manifest
...
def write_grid_attempt(
    *,
    ...
    # The written snapshot -- not the in-memory object -- is what every compiled
    # command loads, so the merge is verified against the file on disk. This is
    # the last line of defence against a lineage whose commands resolve a
    # partial config: the raw-text fallback above, and any future materialization
    # step that drops a choice block, both fail here instead of at run time.
    required_paths = [
        str(path)
        for path in (
            (grid_data.get("required_choice_paths") or ()) if isinstance(grid_data, dict) else ()
        )
    ]
    if required_paths:
        require_choice_paths(
            OmegaConf.load(attempt / snapshots["train"]),
            required_paths,
            context=f"train config snapshot {attempt / snapshots['train']}",
        )
    ...
    validation_snapshot = attempt / snapshots["validation"]
    if required_paths and validation_snapshot.is_file():
        require_choice_paths(
            OmegaConf.load(validation_snapshot),
            required_paths,
            context=f"validation config snapshot {validation_snapshot}",
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Compose the base configs with the shared choice-library fragments they are
    # not self-contained without, and assert the composition before anything is
    # compiled. `load_composed_config` raises when a declared library is absent
    # or a required choice path is still empty after the merge.
    library_specs = choice_library_specs(grid_data.get("choice_libraries"))
    choice_paths = required_choice_paths(grid_data)
    config_obj = load_composed_config(
        config,
        library_specs,
        required_paths=choice_paths,
        repo_root=REPO_ROOT,
        context=f"train config {config}",
    )
    raw_validation_config = grid_data.get("validation_config")
    validation_obj = (
        None
        if raw_validation_config is None
        else load_composed_config(
            raw_validation_config,
            library_specs,
            required_paths=choice_paths,
            repo_root=REPO_ROOT,
            context=f"validation config {raw_validation_config}",
        )
    )
    if library_specs:
        print(
            f"{prefix} merged {len(library_specs)} choice librar"
            f"{'y' if len(library_specs) == 1 else 'ies'} "
            f"({', '.join(spec['path'] for spec in library_specs)}) "
            f"providing {', '.join(choice_paths) or 'no declared paths'}"
        )
    ...
    # Every compiled command points at the grid attempt's own snapshot, blinded
    # or not. Pointing at the source `configs/train.yaml` instead would ship a
    # command whose `choices.basis` is un-merged -- the exact silent-partial
    # failure this study cannot tolerate.
    snapshots = config_snapshot_names(grid_data.get("config_snapshots"))
    attempt_dir = grid_attempt_dir(results_root, attempt_id)
    config_for_jobs: str | Path = attempt_dir / snapshots["train"]
    validation_config_for_jobs: str | Path | None = (
        None if validation_obj is None else attempt_dir / snapshots["validation"]
    )

    points = true_points
    manifest_grid_data = dict(grid_data)
    config_snapshot_data: dict[str, Any] = {"train": config_obj}
    if validation_obj is not None:
        config_snapshot_data["validation"] = validation_obj
    unblind_data: dict[str, Any] | None = None
    ...
    if blinding_enabled(grid_data, args.blind):
        ...
        manifest_grid_data["config"] = str(config_for_jobs)
        if validation_config_for_jobs is not None:
            manifest_grid_data["validation_config"] = str(validation_config_for_jobs)
    manifest_grid_data["choice_libraries"] = library_specs
    manifest_grid_data["required_choice_paths"] = choice_paths
