...
# ---------------------------------------------------------------------------
# Group 2 -- fail-closed divergences.  DISCLOSED AVAILABILITY BOUND.
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    "spelling", sorted(_UNFOLLOWABLE_SPELLINGS), ids=sorted(_UNFOLLOWABLE_SPELLINGS)
)
def test_hi_firewall_review_r2_unfollowable_spelling_refuses_without_execution(
    spelling: str,
) -> None:
    """A spelling the follower cannot read is refused, and nothing runs.

    THIS ARM PINS A DISCLOSED AVAILABILITY BOUND, NOT DESIRED BEHAVIOUR.
    OmegaConf itself resolves every spelling below without difficulty; the
    firewall refuses it because ITS follower cannot, and refusing is the safe
    direction when the alternative is reading a possible HI config as a
    foreign family.  If the follower is ever made grammar-normal, these arms
    MUST BE FLIPPED to assert that the reference is FOLLOWED -- they must not
    be deleted, and they must not be read as a statement that refusing here is
    right.  A green that cements a defect as expected behaviour is the trap
    round 1 caught; this docstring is what keeps this green out of it.
    ...
    cfg = OmegaConf.create(_UNFOLLOWABLE_SPELLINGS[spelling])
    ...
    try:
        ...
        # Restore the real resolver even when the assertion fails: a probe
        # that leaks a witness into the rest of the session would make some
        # later test's result a fact about this one.
        OmegaConf.register_new_resolver(RESOLVER, original, replace=True)

    assert "undeterminable-identity" in _rules(caught.value)
    assert calls == [], "a resolver ran while the identity was being established"
    # The disclosure, asserted rather than only written down: the grammar
    # resolves this spelling, so the refusal is the follower's bound.
    assert OmegaConf.select(cfg, "experiment.name") == FAMILY
