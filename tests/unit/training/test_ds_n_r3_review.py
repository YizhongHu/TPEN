"""Independent, contract-first DS-N round-three evidence."""
...
def run_probe(tmp_path: Path, *, extra_args: tuple[str, ...] = ()):
    return run_gloo_subprocess_group(
        ...
        worker_module="tests.spikes.native_ddp.review_round3_probe",
...
def test_r3_n_g1_m2_observes_uneven_shards_and_global_statistics(tmp_path: Path) -> None:
    result = run_native(tmp_path, world_size=3,
                        extra_args=("--experiment", "scientific", "--fixture", "m2"))
    assert result.publication_observed is True, "R3-ARM-21 worker entrypoint publication"
    assert_success(result)
    observed = states(result)
    assert [len(state["energy"]["__tensor__"]) for state in observed] == [5, 3, 7], "R3-ARM-02 shard lengths"
    energies = [state["energy"]["__tensor__"] for state in observed]
    assert [sum(math.isfinite(value) for value in values) for values in energies] == [3, 0, 5], "R3-ARM-02 local finite counts"
    assert all(math.isclose(energies[rank][index], expected)
               for rank, index, expected in ((0, 0, 1.0), (0, 2, 2.0), (0, 3, -1.0),
                                              (2, 0, 3.0), (2, 1, -2.0), (2, 3, 1.0), (2, 4, 0.5), (2, 5, 2.5))), "R3-ARM-02 exact finite shard values"
    assert [state["global_statistics"]["finite_count"] for state in observed] == [8, 8, 8], "R3-ARM-02 global finite count"
    assert [state["global_statistics"]["total_count"] for state in observed] == [15, 15, 15], "R3-ARM-02 global total count"
    print("R3-M2-SPECIMEN " + json.dumps({
        "state_keys": sorted(observed[0]),
        "energy_lengths": [len(state["energy"]["__tensor__"]) for state in observed],
        "finite_counts": [sum(math.isfinite(value) for value in state["energy"]["__tensor__"]) for state in observed],
        "global_statistics": [state["global_statistics"] for state in observed],
        "global_metrics": [state.get("global_metrics") for state in observed],
        "backend_fields": {key: observed[0].get(key) for key in ("backend", "backend_name", "gloo") if key in observed[0]},
    }, sort_keys=True), flush=True)


def test_r3_n_g1_world_size_compensates_the_ddp_surrogate(tmp_path: Path) -> None:
    ...
    ), "R3-ARM-01 observed ddp_backward_scale must be 2*world_size/M"


def test_r3_n_g1b_rejects_local_centering_against_global_oracle(tmp_path: Path) -> None:
    del tmp_path
    ...
    assert local_centered_objective(logabs, energies).item() == pytest.approx(0.0), "R3-ARM-03 global oracle mismatch"
    assert local_centered_objective(logabs, energies).item() != pytest.approx(
        global_oracle.item()
    ), "R3-ARM-03 global oracle mismatch"


def test_r3_n_g2_all_invalid_has_no_backward_or_parameter_gradient_event(tmp_path: Path) -> None:
    result = run_probe(tmp_path, extra_args=("--experiment", "scientific", "--fixture", "all_invalid"))
    assert result.all_reaped is True
    assert result.publication_observed is False, "R3-ARM-04 all-invalid refuses publication"
    assert result.exit_codes == (0, 0)
    for state in states(result):
        assert state["status"] == "refused"
        assert state["ddp_forward_calls"] == 0, "R3-ARM-22 no DDP forward before refusal"
        assert state["ddp_gradient_reductions"] == 0
        assert state["review_parameter_gradient_events"] == 0, "R3-ARM-04 parameter-gradient event"
        assert state["review_backward_calls"] == 0, "R3-ARM-04 backward event"
        assert state["parameters_before"] == state["parameters_after"], "R3-ARM-04 parameter mutation"
        assert state["counter_before"] == state["counter_after"] == 0, "R3-ARM-04 update counter"
        assert state["optimizer_state_before"] == state["optimizer_state_after"], "R3-ARM-04 optimizer mutation"


def test_r3_reviewer_raw_parameter_backward_positive_control(tmp_path: Path) -> None:
    """The zero-event refusal oracle has a real positive observer control."""
    result = run_probe(tmp_path, extra_args=("--experiment", "scientific"))
    assert_success(result)
    observed = states(result)
    assert all(state["review_backward_calls"] > 0 for state in observed), "R3 positive control observed backward"
    assert all(state["review_parameter_gradient_events"] > 0 for state in observed), (
        "R3 positive control observed raw parameter-gradient event"
    )
    assert all(state["parameter_gradient_events"] > 0 for state in observed), (
        "R3 positive control observed lane parameter-gradient event"
    )


def test_r3_n_g1_clipping_uses_global_not_per_shard_norm() -> None:
    """Opposing shards distinguish global clipping from local clipping."""
    shards = [torch.tensor([3.0]), torch.tensor([-4.0])]
    global_clipped = oracle_global_clip(shards, clip_norm=2.0)
    per_shard_clipped = naive_per_shard_clip_then_sum(shards, clip_norm=2.0)
    assert global_clipped.item() == pytest.approx(-1.0), "R3-G1 global clip oracle"
    assert per_shard_clipped.item() == pytest.approx(0.0), "R3-G1 per-shard clip contrast"
    assert global_clipped.norm().item() <= 2.0, "R3-G1 global clip bound"
...
def test_r3_reviewed_n_g4_fault_tests_preserve_evidence(tmp_path: Path, kind, phase, delay) -> None:
    result = run_native(
    ...
    assert result.publication_observed is False, "R3-G4 no publication after configured fault"
    ...
    ), "R3-G4 configured fault metadata"
    generation = tmp_path / "dcp" / "generations" / "gen-000001"
    assert not (tmp_path / "dcp" / "latest.json").exists(), "R3-G4 no latest publication"
    assert not (generation / "COMPLETE").exists(), "R3-G4 no requested COMPLETE publication"
    assert not generation.exists(), "R3-G4 no final generation publication"
...
def test_r3_reviewer_n_g4_broad_exit_fact_oracle(tmp_path: Path, kind, phase, delay) -> None:
    result = run_native(
        tmp_path, fault_plan=FaultPlan(target_rank=1, kind=kind, phase=phase, delay_seconds=delay),
        bounds=FAULT_BOUNDS, extra_args=("--checkpoint-root", str(tmp_path / "dcp"),
                                         "--checkpoint-generation", "1"),
    ...
    assert any(code != 0 for code in result.exit_codes), "R3-G4 broad exit fact: at least one nonzero child exit"


def test_r3_reviewer_n_g4_broad_exit_fact_oracle_raise(tmp_path: Path) -> None:
    result = run_native(
        tmp_path,
...
def test_r3_n_g4_watchdog_distinguishes_nominal_and_over_bound_stalls(tmp_path: Path) -> None:
    nominal = run_native(
        tmp_path,
    ...
    assert nominal.watchdog_fired is False, "R3-G4 6-second nominal stall stays below watchdog"
    assert nominal.all_reaped is True, "R3-G4 nominal stall all ranks reaped"
    assert nominal.publication_observed is False, "R3-G4 nominal stall no publication"
    ...
    assert over_bound.watchdog_fired is True, "R3-G4 13-second stall exceeds watchdog"
    assert over_bound.all_reaped is True, "R3-G4 over-bound stall is fully reaped"
    assert over_bound.publication_observed is False, "R3-G4 over-bound stall no publication"


def test_r3_reviewer_n_g4_skip_collective_broad_exit_is_clean_and_proven(tmp_path: Path) -> None:
    ...
    assert result.culprit_rank == 1
    assert all(code is not None for code in result.exit_codes)
    print(f"R3-G4 SKIP exact exit tuple={result.exit_codes}")
    assert any(code != 0 for code in result.exit_codes), "R3-G4 broad exit fact: at least one nonzero child exit"
    ...
    assert before == after == expected, "R3-G4 SKIP source/harness digests are clean and unchanged"


def test_r3_n_g3_resume_applies_detached_dcp_buffers(tmp_path: Path) -> None:
    ...
    for current in before:
        ...
        assert current["parameters_before"] == previous["parameters_after"], "R3-ARM-05 resumed parameters"
        assert current["optimizer_state_before"] == previous["optimizer_state_after"], "R3-ARM-05 resumed optimizer state"


def test_r3_n_g3_topology_gate_precedes_all_dcp_and_state_mutation(tmp_path: Path, monkeypatch) -> None:
    ...
    load = Mock(side_effect=AssertionError("R3-ARM-06 DCP load boundary entered"))
    ...
    assert load.call_count == 0, "R3-ARM-06 topology guard blocks DCP load"
    assert apply.call_count == 0
    assert all(torch.equal(model.state_dict()[key], value) for key, value in model_before.items()), "R3-ARM-07 model unchanged"
    assert optimizer.state_dict() == optimizer_before, "R3-ARM-07 optimizer unchanged"


def test_r3_n_g3_integrated_topology_refusal_keeps_pristine_state(tmp_path: Path) -> None:
...
def test_r3_n_g3b_perturbed_rank_sidecar_is_rejected(tmp_path: Path) -> None:
    ...
    failed = run_native(tmp_path, extra_args=("--experiment", "resume", "--iterations", "2",
                                               "--checkpoint-root", str(root), "--resume-generation", "1"))
    assert failed.publication_observed is False, "R3-ARM-08 sidecar validation failure"
    assert all(state["status"] == "resume_failed" for state in states(failed)), "R3-ARM-08 status classification"


def test_r3_n_g5_failed_publication_keeps_previous_generation_selectable(tmp_path: Path) -> None:
    ...
    assert second.publication_observed is False, "R3-G5 failed writer blocks publication"
    assert second.all_reaped is True, "R3-G5 failed writer all ranks reaped"
    assert all(code is not None for code in second.exit_codes), "R3-G5 failed writer exit codes collected"
    assert any(code != 0 for code in second.exit_codes), "R3-G5 failed writer must exit nonzero"
    ...
    assert (root / latest["path"] / "COMPLETE").exists(), "R3-G5 previous generation remains complete"
    assert not (root / "generations" / "gen-000002").exists(), "R3-G5 failed final generation is absent"


def test_r3_n_g5_digest_change_blocks_publication(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        caught = exc
    assert caught is not None, "R3-ARM-13 digest guard did not raise"
    assert "digest changed" in str(caught), "R3-ARM-13 digest mismatch classification"
    assert not (tmp_path / "latest.json").exists(), "R3-G5 digest failure has no latest publication"
    generation = tmp_path / "generations" / "gen-000001"
    assert not (generation / "COMPLETE").exists(), "R3-G5 digest failure has no COMPLETE publication"
    assert not generation.exists(), "R3-G5 digest failure has no final generation publication"


def test_r3_n_g5_delayed_writer_does_not_publish_partial_generation(tmp_path: Path) -> None:
    ...
    assert second.publication_observed is False, "R3-G5 delayed writer blocks publication"
    assert json.loads((root / "latest.json").read_text()) == latest
    assert (root / latest["path"] / "COMPLETE").exists(), "R3-G5 delayed writer preserves prior COMPLETE"
    assert not (root / "generations" / "gen-000002").exists(), "R3-G5 delayed final generation is absent"


def test_r3_n_g6_reduction_count_does_not_scale_with_sampling_work(tmp_path: Path) -> None:
    ...
    for left, right in zip(states(short), states(long), strict=True):
        assert left["ddp_gradient_reductions_per_update"] == 1, "R3-G6 update reduction count"
        assert right["ddp_gradient_reductions_per_update"] == 1, "R3-G6 update reduction count"
        assert left["sampling_gradient_reductions"] == right["sampling_gradient_reductions"] == 0
        assert left["review_semantic_model_forward_calls"] == 6, "R3-G6 executed forward count at one step"
        assert right["review_semantic_model_forward_calls"] == 14, "R3-G6 executed forward count at five steps"


def test_r3_n_e2_coordinate_work_uses_raw_model_only(tmp_path: Path) -> None:
    result = run_native(tmp_path, extra_args=("--kinetic-forwards", "3"))
    assert_success(result)
    for state in states(result):
        assert state["ddp_forward_calls"] == 1, "R3-ARM-16 coordinate work must not use DDP wrapper"
...
def test_r3_n_e3_optimizer_state_and_closure_objective_are_global(tmp_path: Path) -> None:
    ...
    assert observed[0]["parameters_after"] == observed[1]["parameters_after"], (
        "R3-ARM-18 closure parameters must be globally synchronized"
    )
    for state in observed:
    ...
    for state in observed:
        assert_recursive_close(state["parameters_after"], expected_parameters, atol=atol,
                               path="R3-ARM-18 parameters match independent reference")
        assert_recursive_close(
            state["optimizer_state_after"], checkpoint_module._jsonable(reference_optimizer.state_dict()),
            atol=atol, path="R3-E3 recursive optimizer state matches global LBFGS reference",
        )


def test_r3_n_e3_sgd_and_adam_have_nonempty_independent_optimizer_evidence(tmp_path: Path) -> None:
    for name in ("sgd", "adam"):
        ...
        for state in observed:
            assert_recursive_close(state["parameters_after"], expected_parameters, atol=atol,
                                   path=f"R3-ARM-17 {name} parameters match independent reference")
            assert_recursive_close(
                state["optimizer_state_after"], checkpoint_module._jsonable(optimizer.state_dict()),
                atol=atol, path=f"R3-E3 {name} complete optimizer state",
            )


def test_r3_n_e4_inventory_names_consumed_state_dict_helpers_and_classifies_them(tmp_path: Path) -> None:
    ...
    assert consumed <= set(inventory["experimental"]), "R3-ARM-19 consumed DCP API inventory"
    assert set(inventory["stable"]).isdisjoint(consumed)
    assert states(result)[0]["accelerator_execution"] is False


@pytest.mark.parametrize("api", ("torch.distributed.checkpoint.save", "torch.distributed.checkpoint.load"))
def test_r3_n_e4_consumed_save_load_inventory_oracle(api: str, tmp_path: Path) -> None:
    result = run_native(tmp_path)
    assert_success(result)
    inventory = states(result)[0]["api_inventory"]
    ...
    assert locations == ["stable"], (
        f"R3-E4 consumed API {api} must appear in stable inventory only; observed={locations}"
    )


def test_r3_n_e5_state_and_receipt_are_rank_attributed(tmp_path: Path) -> None:
    result = run_native(tmp_path)
    assert_success(result)
    for receipt, state in zip(result.receipts, states(result), strict=True):
        ...
        assert "pid" in state, "R3-ARM-20 pid field"
        assert state["pid"] == receipt.pid, "R3-ARM-20 rank pid attribution"
        assert state["hostname"] == receipt.hostname
        assert state["phase_sequence"] == receipt.phase_sequence


def test_r3_checkpoint_load_uses_no_state_application_on_topology_mismatch(tmp_path: Path) -> None:
